...
class Campaign {
    ...
    async _scrapeData () {
        ...
        while (nextLink) {
            ...
            allPatronsSocials = response.included ? [ ...allPatronsSocials, ... response.included ] : undefined;
        }
    ...
    async lookForPatron (discordId) {

        const url = `https://www.patreon.com/api/oauth2/v2/campaigns/${this.campaignId}/members?include=user&fields%5Buser%5D=social_connections`
        ...
        let discord = response.included.find(patron => patron?.attributes?.social_connections?.discord.user_id == discordId);
        if (discord) {
            let userId = discord.id;
            let patron = response.data.find(patron => patron?.relationships?.user?.data?.id == userId);
            if (patron) {
                return await this.fetchPatronById(patron.id);
            } else {
                return undefined
            }
        } else {
    ...
    async fetchPatronById (pledgeId) {
